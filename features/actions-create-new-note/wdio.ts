...
async function visibleModalContaining(text: string): Promise<WdioElement> {
  try {
      ...
      async () =>
        await browser.execute(
          (expected, captureAttribute) => {
            document
              .querySelectorAll(`[${captureAttribute}]`)
              .forEach((element) => {
                element.removeAttribute(captureAttribute);
              });
            const container = [
              ...
              (modal) =>
                ...
                modal.textContent?.includes(expected),
            );
            if (!container) return false;
            const surface =
              [
                ...container.querySelectorAll<HTMLElement>(".modal, .prompt"),
    ...
    const state = await browser.execute(() => ({
      ...
      notices: [...document.querySelectorAll<HTMLElement>(".notice")].map(
...
async function waitForCreatedFile(filePath: string): Promise<void> {
  await browser.waitUntil(
    async () => {
      try {
        return (
          (await obsidianPage.read(filePath)) === "---\nstatus: open\n---\n"
        );
      } catch {
        return false;
      }
...
async function closeModalIfOpen(): Promise<void> {
  const open = await browser.execute(() =>
    [...document.querySelectorAll<HTMLElement>(".modal-container")].some(
...
export const { acceptanceScenarios, screenshotScenarios } = featureScenarios(
  creationFixture,
  {
    acceptance: [
      {
        title:
          "creates and opens an initialized note in the selected notes folder",
        async run() {
          try {
            const folderModal = await openFolderPicker();
            await expect(folderModal).toHaveText(
              expect.stringContaining(TARGET_FOLDER),
            );
            await expect(folderModal).toHaveText(
              expect.not.stringContaining("Outside"),
            );
      ...
      },
      {
        ...
        async run() {
          try {
            ...
            await expect(inbox).toHaveText(
              expect.stringContaining(ROOT_VIOLATION),
            );
          } finally {
            await closeModalIfOpen();
          }
