    ...
    async () =>
      await browser.execute(
        (fileName, expectedText) => {
          ...
          const link =
            action?.querySelector<HTMLAnchorElement>("a.internal-link");
          return (
            action?.textContent?.trim() === expectedText &&
...
export const { acceptanceScenarios, screenshotScenarios } = featureScenarios(
  markdownFixture,
  {
    acceptance: [
      {
        title:
          "renders task links and inline formatting through Obsidian Markdown",
        async run() {
