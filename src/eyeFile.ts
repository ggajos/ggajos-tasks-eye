...
function parseValue(raw: string): unknown {
  ...
  if (/^\[\[[^\]]+\]\]$/.test(trimmed)) return parseScalar(trimmed);
...
export function buildEyeFileFromMarkdown(
  ...
  if (Object.getOwnPropertyDescriptor(frontmatter, "up") !== undefined) {
...
export function buildEyeFilesFromMarkdown(
  sources: readonly MarkdownFileSource[],
  managedFolderPath = DEFAULT_MANAGED_FOLDER_PATH,
): EyeFile[] {
  const files = sources.map((source) =>
    buildEyeFileFromMarkdown(
      source.path,
      source.markdown,
      source.frontmatter ?? parseFrontmatter(source.markdown),
      managedFolderPath,
    ),
  );
