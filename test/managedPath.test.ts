...
describe("managed notes folder paths", () => {
  ...
  it("recognizes parent folder changes without following renames", () => {
    expect(
      isPathRelatedToManagedFolder("Workspace", "Workspace/Projects"),
    ).toBe(true);
    expect(
      isPathRelatedToManagedFolder(
    ...
    expect(
      isPathRelatedToManagedFolder("Elsewhere", "Workspace/Projects"),
    ).toBe(false);
  });
});
